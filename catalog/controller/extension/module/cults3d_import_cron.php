...
class ControllerExtensionModuleCults3dImportCron extends Controller {
    public function index() {
        ...
        $batch = isset($this->request->get['batch']) ? trim($this->request->get['batch']) : '';
        ...
        $ow_fields = [
            'name'        => $this->config->get('module_cults3d_import_ow_name') ? true : false,
            'description' => $this->config->get('module_cults3d_import_ow_description') ? true : false,
            'meta'        => $this->config->get('module_cults3d_import_ow_meta') ? true : false,
            'images_mode' => $this->config->get('module_cults3d_import_ow_images_mode') ?: 'append',
            'category'    => $this->config->get('module_cults3d_import_ow_category') ? true : false
        ];
        ...
        for ($i=0; $i<$limit; $i++) {
            $sql = "SELECT * FROM `" . DB_PREFIX . "cults3d_queue` WHERE `status`='pending'";
            if ($batch !== '') {
                $sql .= " AND `batch`='" . $this->db->escape($batch) . "'";
            }
            $sql .= " ORDER BY `queue_id` ASC LIMIT 1";
